...
''',
  ...
  test('Android stock launcher icon adds a splash warning', () {
    _writeProject(
      tempDirectory,
      pubspec: '''
name: demo_app
...
''',
    ...
    _writeAndroidLauncherIcon(tempDirectory, _stockAndroidLauncherIconBytes);

    final NativeLensSplashPlan plan = buildSplashPlan(
      workingDirectory: tempDirectory,
    );

    expect(plan.warnings, contains(_androidDefaultLauncherIconWarning));
  });

  test('Android customized launcher icon does not add stock warning', () {
    _writeProject(
      tempDirectory,
      pubspec: '''
name: demo_app
...
''',
    ...
    _writeAndroidProject(tempDirectory);
    _writeAndroidLauncherIcon(tempDirectory, <int>[9, 8, 7, 6]);

    final NativeLensSplashPlan plan = buildSplashPlan(
      workingDirectory: tempDirectory,
    );

    expect(plan.warnings, isNot(contains(_androidDefaultLauncherIconWarning)));
  });

  test('missing Android launcher icon does not add stock warning', () {
    _writeProject(
      tempDirectory,
      pubspec: '''
name: demo_app
...
''',
  ...
  test('iOS stock AppIcon adds a splash warning', () {
    _writeProject(
      tempDirectory,
      pubspec: '''
name: demo_app
...
''',
    ...
    _writeIosMarketingIcon(tempDirectory, _stockIosMarketingIconBytes);

    final NativeLensSplashPlan plan = buildSplashPlan(
      workingDirectory: tempDirectory,
    );

    expect(plan.warnings, contains(_iosDefaultAppIconWarning));
  });

  test('iOS customized AppIcon does not add stock warning', () {
    _writeProject(
      tempDirectory,
      pubspec: '''
name: demo_app
...
''',
    ...
    _writeIosProject(tempDirectory);
    _writeIosMarketingIcon(tempDirectory, <int>[6, 7, 8, 9]);

    final NativeLensSplashPlan plan = buildSplashPlan(
      workingDirectory: tempDirectory,
    );

    expect(plan.warnings, isNot(contains(_iosDefaultAppIconWarning)));
  });

  test('missing iOS AppIcon does not add stock warning', () {
    _writeProject(
      tempDirectory,
      pubspec: '''
name: demo_app
...
''',
  ...
  test('dry-run output includes stock launcher icon warnings', () async {
    _writeProject(
      tempDirectory,
      pubspec: '''
name: demo_app
...
''',
    ...
    expect(exitCode, 0);
    expect(output.join('\n'), contains(_androidDefaultLauncherIconWarning));
    expect(output.join('\n'), contains(_iosDefaultAppIconWarning));
    expect(
      Directory(
        [
          tempDirectory.path,
...
void _writeAndroidLauncherIcon(Directory directory, List<int> bytes) {
  File(
      [
        directory.path,
        'android',
...
void _writeIosMarketingIcon(Directory directory, List<int> bytes) {
  File(
      [
        directory.path,
        'ios',
...
List<int> get _stockAndroidLauncherIconBytes =>
    _decodeBase64Png(_stockAndroidLauncherIconBase64);

List<int> get _stockIosMarketingIconBytes =>
    _decodeBase64Png(_stockIosMarketingIconBase64);

List<int> _decodeBase64Png(String value) {
  return base64Decode(value.replaceAll(RegExp(r'\s+'), ''));
}

const String _androidDefaultLauncherIconWarning =
...
const String _iosDefaultAppIconWarning =
...
const String _stockAndroidLauncherIconBase64 = '''
